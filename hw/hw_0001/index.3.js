...
currencies.forEach((currency) => {
  console.log(`- ${currency.name} (${currency.shortname}) `);
  console.log(`${currency.amount}x${currency.rate}`.padStart(30, " "));
});

var totalCost = currencies.reduce((prev, currency) => {
  return prev + currency.amount*currency.rate;
}, 0);

console.log(`\nTotal:${totalCost.toFixed(2).padStart(20, " ")} UAH`.toUpperCase());